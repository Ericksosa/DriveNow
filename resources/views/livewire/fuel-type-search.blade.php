<div>
    <div class="py-12">
            ...
            <div class="p-6 flex justify-between items-center">
                <div>
                    <h3 class="text-lg font-semibold text-gray-800 dark:text-gray-200">
                        {{ __('Tabla Gestión de Tipos de Combustibles') }}
                    </h3>
                    <p class="text-gray-600 dark:text-gray-400">
                        {{ __('Aqui puedes gestionar los tipos de combustibles. Crear, Editar y Eliminar') }}</p>
                </div>
                <div class="flex relative">
                    ...
                    <a href="{{ route('fuel-type.export') }}" id="export_button"
                    ...
                    <a href="{{ route('fuel-type.export.pdf') }}" id="export_button"
                        ...
                        <tr>
                            ...
                            @forelse($fuelTypes as $fuelType)
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm">
                                    ...
                                    <a href="{{ route('fuel-type.edit', ['fuel_type' => $fuelType->id]) }}"
                                        class="px-4 py-2 text-blue-500 hover:text-blue-700">
                                        {{ __('Editar') }}
                                    </a>
                                    <form method="POST"
                                        action="{{ route('fuel-type.destroy', ['fuel_type' => $fuelType->id]) }}"
                                        style="display: inline;" class="delete-form">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="px-4 py-2 text-red-500 hover:text-red-700"
                                            data-confirm="¿Estás seguro de que deseas eliminar el tipo de combustible {{ $fuelType->name }}?">
                                            {{ __('Eliminar') }}
                                        </button>
                                    </form>
                                </td>
        ...
        @if ($search == '')
            <div class="m-4">
                {{ $fuelTypes->links('pagination::tailwind') }}
            </div>
        @endif
    </div>
</div>
